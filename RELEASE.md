...
* **Fix**: Improved documentation about the number format when dialing and adding participants to a conference.
* **Fix**: Wrong media type (video) shown when making calls from the browser to mobile devices.
* **New**: New method `Phone.rejectModification`. This method allows the  user to reject a media modification requests such as upgrade requests generated by the `Phone.upgrade` method or downgrade requests generated by the `Phone.downgrade` method.
* **New**: Automatically reject call modification if its not accepted after 5s in Sample app.

## Known Issues

* Moving call from Mobile Number (Browser) to Mobile device results in downgrade offer. 
